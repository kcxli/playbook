...
def build_draft(reports: list[dict[str, Any]], args: argparse.Namespace) -> str:
    ...
    lines.extend(
        [
            ...
            "# Navigation and submit-like buttons are commented unless explicitly enabled.",
            "",
            "steps:",
        ]
    )
...
def emit_button_candidates(lines: list[str], report: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    for button in sorted(buttons, key=sort_key):
        ...
        risky = bool(button.get("likely_navigation_or_submit")) or bool(SUBMIT_BUTTON_RE.search(text))
        ...
        comments.append("button left commented; confirm ordering/navigation before enabling")
        if risky:
            comments.append("navigation/submit-like button")
